...
XQuery est un langage fortement typé. La signature des fonction XQuery facilite le débugage et permet leur documentation. En plus de ce typage, l’utilisation de [xqDoc](http://xqdoc.org) permet de rendre votre code encore plus explicite.
...
Tout en essayant d’être le plus succinct possible, définissez des règles de rédaction pour ces descriptions (temps, forme verbale des phrases, etc.)
...
Dans bien des cas, il est possible de simplifier le code en évitant la répétition de la même expression soit au niveau de l’expression sur laquelle on itère, soit en utilisant la clause let.
